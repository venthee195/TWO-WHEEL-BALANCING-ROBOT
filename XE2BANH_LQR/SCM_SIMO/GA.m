...
max_generation=200; %so the he toi da trong qua trinh chay(chay toi toi da 200 the he thi dung lai)
...
range=[-10 -10   -10  -10    ;...
        10 10  10   10 ]; %tam cua cac NST
dec=[1 1 1  1 ];  %vi tri dau cham thap phan
sig=[4 4 4  4 ];  %so chu so co nghia trong moi nhiem sac the

cross_prob = 0.2; %he so lai ghep
mutate_prob = 0.8; %he so dot bien(he so dot bien + he so lai ghep =1)
